...
const handleContestCheck=(req,res,db)=> {
	...
	.then(async (probs)=>{
				...
				if(req.body.lang==='c')
					...
					resultPromise = node.runSource(sourcecode,{ stdin:`${probs.actual_ip}` });
				else
				...
				if((result.stderr))
					...
					if(req.body.mark===1){
						...
						if(flag===0){
							db("correct").where("u_id","=",req.body.u_id)
							.andWhere("p_id","=",req.body.p_id).then((data)=>{if (data.length==0){db("correct").insert({u_id:req.body.u_id,p_id:req.body.p_id,c_id:req.body.c_id,score:req.body.score }).then(()=>{})}})
							.catch(err => res.send(err));
							db("submissions").insert({u_id:req.body.u_id,c_id:req.body.c_id,p_id:req.body.p_id,corr_status:1,score:req.body.score }).then(()=>{});
							res.json("correct answer");
						}
						else{
							res.json("wrong answer");
